...
var DAILY_GLOSSARIES = {
  "2026-08-31": {
    ...
    "domino": { english: "dominoes", explanation: "Dominoes is a major part of everyday social life in Cura\xE7ao, played outdoors in neighborhoods and at family gatherings, often with real competitive spirit, not just casual fun." },
    "d\xF9im": { english: "thumb", explanation: 'The body part. "He hurt his thumb closing the door."' },
    "d\xF9in": { english: "dune", explanation: 'A sand mound near the beach. "We walked over the dune to reach the shore."' },
    ...
    "mid\xED": { english: "measurement, size", explanation: `The actual measurement or size that results. "What's the measurement of this room?"` },
    ...
    "min\xEDmo": { english: "minimum, least", explanation: `The smallest possible amount. "That's the minimum you need to pass."` },
    ...
    "sin\xF3nimo": { english: "synonym", explanation: `When two words mean the same thing. "'Happy' and 'glad' are synonyms."` },
    "sin\xF9s": { english: "sinus", explanation: 'The sinus cavity in the face. "My sinuses are blocked from the cold."' },
    ...
    "unis\xF3no": { english: "unison", explanation: 'When everyone sings or plays the same note together. "The choir sang the last line in unison."' }
  }
};
function normaliseGlossaryKey(value) {
  return String(value).normalize("NFD").replace(/[\u0300-\u036f]/g, "").normalize("NFC").toLowerCase();
}
__name(normaliseGlossaryKey, "normaliseGlossaryKey");
...
${text}`;
  ...
  async getPuzzlePreview(dateString) {
    ...
    const normalise = /* @__PURE__ */ __name((value) => [...String(value).normalize("NFC").toLowerCase().trim()].map((letter) => fold[letter] || letter).join(""), "normalise");
    const letterBit = /* @__PURE__ */ __name((letter) => letter === "\xF1" ? 1 << 26 : 1 << letter.charCodeAt(0) - 97, "letterBit");
    const countBits = /* @__PURE__ */ __name((mask) => {
      let count = 0;
      while (mask) {
        mask &= mask - 1;
        count++;
    ...
    const number = Math.round((Date.parse(`${dateString}T00:00:00Z`) - epoch) / 864e5);
  ...
  async sendTomorrowPuzzleEmail(date = (/* @__PURE__ */ new Date()).toISOString().slice(0, 10)) {
    ...
    const html = `<!doctype html><html><body style="margin:0;background:#f4f1ea;font-family:Arial,sans-serif;color:#081f36"><main style="max-width:700px;margin:24px auto;padding:24px;background:#fff;border:1px solid #ddd6c9"><h1 style="font-size:22px;margin:0 0 18px">Tomorrow's Palabra di K\xF2rsou puzzle</h1><p><strong>Date:</strong> ${date}<br><strong>Letters:</strong> ${puzzle.letters.join(" ").toUpperCase()}<br><strong>Center:</strong> ${puzzle.centre.toUpperCase()}<br><strong>Pangram:</strong> ${escapeHtmlServer(puzzle.pangrams.join(", "))}</p><p>Write the Papiamento definitions, paste them into <code>DEFINITIONS</code> in <code>public/index.html</code>, then deploy.</p><pre style="white-space:pre-wrap;overflow-wrap:anywhere;padding:16px;background:#f4f1ea;border:1px solid #ddd6c9">${escapeHtmlServer(definitionTemplate)}</pre></main></body></html>`;
    await this.sendNotificationEmail(
      `[Palabra di K\xF2rsou] Definitions for ${date}`,
      text,
      html
    );
  }
...
`;
  ...
  async handleDefine(request) {
    try {
      ...
      const dailyEntry = dailyGlossary && (dailyGlossary[display] || dailyGlossary[word] || Object.entries(dailyGlossary).find(
        ([entryWord]) => normaliseGlossaryKey(entryWord) === normaliseGlossaryKey(display)
      )?.[1]);
      if (dailyEntry) {
        if (!this.env.GOOGLE_TRANSLATE_API_KEY) {
          return json({ error: "Google Translate no ta konfigur\xE1" }, 503);
        }
      ...
      if (cached && cached.definition_source === "google" && cached.translation_source === "google" && cached.pipeline_version === 3) {
        return json({ word, ...cached, definitionNl: cached.definition, cached: true }, 200);
      }
...
If unsure, leave all fields empty ("").`;
  ...
  async resolveDefinition(word, display, tags) {
    ...
    if (this.env.GOOGLE_TRANSLATE_API_KEY && this.env.ANTHROPIC_API_KEY) {
      const googleMeaning2 = await this.googleTranslateWord(
        displayWord
      );
      if (googleMeaning2?.meaning) {
        const result = await this.generateDefinition(
          normalWord,
          displayWord,
          tags,
          googleMeaning2.meaning
        );
        if (result?.definition) {
          return {
            ...result,
            ...
            target_language: "en",
            verification_status: "automatic",
            english: googleMeaning2.meaning,
            needs_review: 1,
            translation_ambiguous: googleMeaning2.needsReview ? 1 : 0
          };
        }
      } else {
        console.log(
  ...
  async ensureUsersTable() {
    ...
    try {
      await this.env.GAME_HISTORY.prepare("ALTER TABLE users ADD COLUMN email TEXT").run();
    } catch {
    }
    try {
      await this.env.GAME_HISTORY.prepare("ALTER TABLE users ADD COLUMN password_reset_token_hash TEXT").run();
    } catch {
    }
    try {
      await this.env.GAME_HISTORY.prepare("ALTER TABLE users ADD COLUMN password_reset_expires_at TEXT").run();
    } catch {
    }
    await this.env.GAME_HISTORY.prepare(
      ...
      const result = await this.env.GAME_HISTORY.prepare(
        `UPDATE users
           ...
           WHERE id = ?
             ...
             AND password_reset_expires_at > datetime('now')`
      ).bind(hashedPassword, salt, user.id, tokenHash).run();
      if (!result.meta.changes) {
        return json({ error: "Invalid or expired reset link" }, 400);
      }
      await this.ensureAuthSessionsTable();
      await this.env.GAME_HISTORY.prepare("DELETE FROM auth_sessions WHERE user_id = ?").bind(user.id).run();
      return json({ ok: true }, 200);
    } catch (e) {
  ...
  async handleSignup(request) {
    try {
      ...
      try {
        ...
        if (String(e).toLowerCase().includes("users.email")) {
          return json(
            {
  ...
  <div class="cf-auth-dialog">
    ...
    <form id="cf-auth-form" class="cf-auth-fields">
      ...
      <div id="cf-email-hint" style="display:none;padding:10px 12px;background:var(--line);border-radius:8px;font-size:12px;color:var(--dim);line-height:1.4;">
        Esaki parse un email \u2014 bo ke usa "<span id="cf-email-hint-value"></span>" komo bo email di rekuperashon i skohe un otro n\xF2mber di uzuario?
        <button type="button" id="cf-use-as-email-btn" style="display:block;margin-top:6px;background:none;border:none;color:var(--flag);font-weight:700;font-size:12px;cursor:pointer;padding:0;text-decoration:underline;">S\xED, usa dje</button>
      </div>
      <input class="name-input" id="cf-auth-email" type="email" autocomplete="email" placeholder="Email (opshonal, pa rekuper\xE1 bo kuenta)" style="display:none;">
...
// .wrangler/tmp/bundle-vtltRg/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-vtltRg/middleware-loader.entry.ts
